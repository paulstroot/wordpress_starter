...
<div class="relative <?php echo esc_attr( $class_name ); ?>" style="<?php echo esc_attr( $style ); ?>">
  <div class="absolute inset-0 z-[1]" style="background-color: <?php echo $background_overlay; ?>"></div>
  <div class="relative z-[2] p-3">
    <h1 class=""><?php echo esc_html( $headline ); ?></h1>

    <?php foreach ( $links as $link ) : ?>
      <a href="<?php echo esc_url( $link['link_url'] ); ?>" target="_blank" class="text-xxl">
        <span class="dashicons <?php echo $link['link_icon']; ?>"></span>
        <span class="sr-only visually-hidden"><?php echo esc_html( $link['link_name'] ); ?></span>
      </a>
    <?php endforeach; ?>
  </div>
